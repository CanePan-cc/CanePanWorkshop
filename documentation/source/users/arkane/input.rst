...
There are two different types available: Petersson-type, which fits one parameter for each bond type, and Melius-type,
...
``fit_mol_corr``       No                   Fit the optional molecular correction term (Melius only, default: False)
